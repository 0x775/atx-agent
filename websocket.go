...
func NewSockServer() *WebSocketServer {
	server := &WebSocketServer{}
	//server.initHTTPServer()
	return server
}

func (server *WebSocketServer)initServer(){
	// ws://127.0.0.1:8888/ws
	c, _, err := websocket.DefaultDialer.Dial("ws://192.168.5.107:8888/ws", nil)
	...
	//心跳包,间隔60秒发送心跳包
	for {
		time.Sleep(time.Duration(1*30)*time.Second)
